...
def dashboard_inventario(request):
    ...
    for r in rutas_db:
        ...
        # IMPORTANTE: Usamos max() porque en el archivo la distancia total se repite por segmento
        distancia_ruta_m = max((t.distancia_m or 0 for t in tramos), default=0)
        if not distancia_ruta_m and r.distancia_m:
            distancia_ruta_m = r.distancia_m
        if not distancia_ruta_m:
            coords = list(r.coordenadas.all().order_by('orden'))
            if len(coords) >= 2:
                distancia_ruta_m = calculate_coordinate_distance(coords)
                # Persistir en la ruta para evitar cálculos futuros
                r.distancia_m = distancia_ruta_m
                r.save(update_fields=['distancia_m'])
                
        distancia_ruta_km = round(distancia_ruta_m / 1000, 2)
        
        # Reservas de esta ruta (evitamos duplicados de tramos)
        res_tramos_m = max((t.reservas_m or 0 for t in tramos), default=0)
...
def update_detalle_fibra(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Update directo en DB para saltar posibles bloqueos de instancia
            updated_count = InventarioFibra.objects.filter(id=fibra_id).update(
                fibra_numero=str(data.get('fibra_numero', '')).strip(),
                estado=nuevo_estado,
                nombre_fibra=str(data.get('nombre_fibra', '')).strip(),
                origen_odf=str(data.get('origen_odf', '')).strip(),
            ...
            # Recalcular conteo de hilos para la ruta
            fibra = InventarioFibra.objects.get(id=fibra_id)
            if fibra.ruta_id:
                from ..models import InventarioTramo
                ...
                InventarioTramo.objects.filter(ruta_id=fibra.ruta_id).update(
...
def get_detalle_puertos(request, odf_nombre):
    ...
    query = DetallePuertoODF.objects.filter(odf=odf_nombre)
    if hub_site:
...
def update_detalle_puerto(request):
    ...
    if request.method == 'POST':
        try:
            ...
            print(f"\n[DEBUG UPDATE PUERTO] ID: {puerto_id}")
            print(f"[DEBUG UPDATE PUERTO] Datos recibidos: {data}")
            
            # Update directo en DB para evitar problemas de instancia
            updated_count = DetallePuertoODF.objects.filter(id=puerto_id).update(
                bandeja=str(data.get('bandeja', '')).strip(),
                puerto_odf=nuevo_puerto,
                fibra=str(data.get('fibra', '')).strip(),
                estado_puerto=nuevo_estado,
                tipo_conector=str(data.get('tipo_conector', '')).strip(),
                patchcord=str(data.get('patchcord', '')).strip(),
                destino=str(data.get('destino', '')).strip(),
            ...
            # Obtener el puerto actualizado para recalcular
            puerto_actualizado = DetallePuertoODF.objects.get(id=puerto_id)
            
            # Recalcular conteos para el ODF usando el foreign key si existe, o el nombre de texto
            odf_obj = None
            if puerto_actualizado.odf_obj:
                odf_obj = puerto_actualizado.odf_obj
            else:
                odf_obj = InventarioODF.objects.filter(odf=puerto_actualizado.odf).first()
            ...
            if odf_obj:
                if puerto_actualizado.odf_obj:
                    ...
                    ocupados = DetallePuertoODF.objects.filter(odf=odf_obj.odf, estado_puerto='Ocupado').count()
                    libres = DetallePuertoODF.objects.filter(odf=odf_obj.odf, estado_puerto='Libre').count()
                    
                InventarioODF.objects.filter(id=odf_obj.id).update(
            ...
            return JsonResponse({
                ...
                "debug_info": f"ID={puerto_id}, Estado={nuevo_estado}, Filas Afectadas={updated_count}"
            })
        except Exception as e:
            return JsonResponse({"status": "error", "message": str(e)}, status=400)
    return JsonResponse({"status": "error", "message": "Método no permitido"}, status=405)
...
def create_odf_manual(request):
    ...
    try:
        ...
        # Validación de duplicado (Hub + Sala + Rack + Nombre)
        exists = InventarioODF.objects.filter(
            hub_site=hub,
            sala=sala,
            rack=rack,
            odf=odf_nombre
...
def create_puerto_manual(request):
    ...
    try:
        ...
        # Find the ODF
        odf_obj = InventarioODF.objects.filter(odf=odf_nombre).first()
        if not odf_obj:
        ...
        # Actualizar contadores del ODF
        odf_obj.capacidad_puertos = (odf_obj.capacidad_puertos or 0) + 1
        if estado_puerto == 'Libre':
            odf_obj.puertos_libres = (odf_obj.puertos_libres or 0) + 1
        elif estado_puerto == 'Ocupado':
            odf_obj.puertos_ocupados = (odf_obj.puertos_ocupados or 0) + 1
        odf_obj.save()
...
def update_odf_manual(request):
    ...
    try:
        ...
        if (odf.hub_site != hub or odf.sala != sala or odf.rack != rack or odf.odf != odf_nombre):
            exists = InventarioODF.objects.filter(hub_site=hub, sala=sala, rack=rack, odf=odf_nombre).exclude(id=id_odf).exists()
            if exists:
...
def get_puertos_odf_api(request):
    ...
    if not odf:
        odf = InventarioODF.objects.filter(odf=odf_nombre).first()
        
    if not odf:
        return JsonResponse({'status': 'error', 'message': f'No se encontró el ODF {odf_nombre}'})
        
    puertos = list(DetallePuertoODF.objects.filter(odf=odf))
    ...
    ruta_asociada = Ruta.objects.filter(tramos_inventario__odf_nombre=odf.odf).first()
...
def planta_interna_view(request):
    """Vista global para Planta Interna (Todos los Puertos ODF)."""
    from ..models import DetallePuertoODF
    # Seleccionamos información relevante
    # Para optimizar, podríamos pasar los datos como JSON o pasarlos en el contexto
    puertos = list(DetallePuertoODF.objects.select_related('odf_obj').values(
        'id', 'odf', 'bandeja', 'puerto_odf', 'fibra', 'estado_puerto', 
        'tipo_conector', 'patchcord', 'destino', 'odf_obj__hub_site'
    ))
    
    from django.core.serializers.json import DjangoJSONEncoder
    import json
    context = {
        'puertos_json': json.dumps(puertos, cls=DjangoJSONEncoder)
    }
    return render(request, 'mapa_inventario/planta_interna.html', context)

@login_required
def planta_externa_view(request):
    """Vista global para Planta Externa (Hilos y Reservas)."""
    from ..models import InventarioFibra, Reserva, InventarioTramo
    
    # Pre-calcular origen de ruta
    tramos = InventarioTramo.objects.filter(tramo_secuencia=1).values('ruta_id', 'odf_nombre', 'hub_site', 'origen')
    ruta_origen_map = {}
    for t in tramos:
        origen = "-"
        if t['odf_nombre'] and t['odf_nombre'].strip() and t['odf_nombre'] != "N/A":
            origen = t['odf_nombre']
        elif t['hub_site'] and t['hub_site'].strip() and t['hub_site'] != "N/A":
            origen = t['hub_site']
        elif t['origen'] and t['origen'].strip() and t['origen'] != "N/A":
            origen = t['origen']
        ruta_origen_map[t['ruta_id']] = origen

    fibras_qs = InventarioFibra.objects.select_related('ruta').values(
        'id', 'ruta_id', 'ruta__nombre', 'fibra_numero', 'estado', 'origen_odf', 'destino', 'nombre_fibra'
    )
    
    fibras = []
    for f in fibras_qs:
        f_dict = dict(f)
        f_dict['origen_odf_dinamico'] = ruta_origen_map.get(f['ruta_id'], "-")
        fibras.append(f_dict)
    
    reservas = list(Reserva.objects.select_related('ruta').values(
        'id', 'ruta__nombre', 'nombre', 'reserva_m', 'latitud', 'longitud', 'tipo'
    ))
    
    from django.core.serializers.json import DjangoJSONEncoder
    import json
    context = {
        'fibras_json': json.dumps(fibras, cls=DjangoJSONEncoder),
        'reservas_json': json.dumps(reservas, cls=DjangoJSONEncoder)
    }
