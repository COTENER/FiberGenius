...
def actualizar_medicion_individual(request):
    ...
    api_url = f"{settings.MEDICION_INDIVIDUAL_API_URL}/{id_onmsi}/measurements/start"
    ...
    try:
        ...
        if response.status_code in [200, 201, 202, 204]:
            ...
            return JsonResponse({'status': 'error', 'message': f'La API respondió con error {response.status_code}: {response.text}'}, status=502)

    except requests.exceptions.RequestException as e:
        logger.error(f"Error de conexión con equipo de medición: {e}")
        return JsonResponse({'status': 'error', 'message': f'Error de conexión con el equipo de medición: {str(e)}'}, status=503)
