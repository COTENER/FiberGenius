...
def crear_usuario(request):
    if request.method == 'POST':
        form = CustomUserCreationForm(request.POST, actor=request.user)
        if form.is_valid():
            form.save()
...
def editar_usuario(request, pk):
    ...
    if request.method == 'POST' and 'update_profile' in request.POST:
        form_profile = CustomUserChangeForm(request.POST, instance=usuario, actor=request.user)
...
def obtener_permisos_agrupados():
    ...
    for p in permisos:
        ...
        if code == 'can_view_reports':
            ...
            'inventariotramo', 'inventariofibra', 'cablefibra', 'tramocable',
            'fibracable', 'coordenadatramo', 'elementoplantaexterna', 'empalmefibra',
        ]:
            agrupados["Inventario"]["Inventario Externo"].append(p)
            agrupados["Inventario"]["Planta Externa"].append(p)
        elif modelo_key in [
        ...
        elif modelo_key in ['site', 'hub', 'nodos', 'hubsite', 'sitealias']:
            agrupados["Administración"]["Sites / Hubs"].append(p)
        elif modelo_key == 'loteimportacion':
            agrupados["Administración"]["Importación"].append(p)
