...
def ranking(request):
    ...
    with connection.cursor() as cursor:
        ...
        top_rutas_query = f"""
            SELECT COALESCE(r.nombre, a.route_name), COALESCE(MAX(o.nombre), MAX(a.device_serial)) as device_serial,
                SUM(CASE WHEN a.alarm_type IN ('High Loss', 'Event Loss') THEN 1 ELSE 0 END) AS attenuation_count,
                SUM(CASE WHEN a.alarm_type = 'INJECTION' THEN 1 ELSE 0 END) AS injection_count,
                SUM(CASE WHEN a.alarm_type IN ('Reflective', 'Event Reflectance', 'REFLEXION') THEN 1 ELSE 0 END) AS reflexion_count,
                SUM(CASE WHEN a.alarm_type = 'Fiber Break' AND (a.status != 'Resolved' OR EXTRACT(EPOCH FROM (CAST(a.fecha_resolucion AS TIMESTAMP) - CAST(a.timestamp AS TIMESTAMP)))/60 > 60) THEN 1 ELSE 0 END) AS fiber_cut_count,
                SUM(CASE WHEN a.alarm_type = 'Fiber Break' AND a.status = 'Resolved' AND EXTRACT(EPOCH FROM (CAST(a.fecha_resolucion AS TIMESTAMP) - CAST(a.timestamp AS TIMESTAMP)))/60 <= 60 THEN 1 ELSE 0 END) AS disconnections_count,
                COUNT(*) as total_count
            FROM mon_alarmas_veex a
            LEFT JOIN inv_rutas_fibra r ON r.id = a.ruta_obj_id
            LEFT JOIN inv_equipos_otu o ON o.id = r.otu_id
            {where_clauses}
            GROUP BY COALESCE(r.nombre, a.route_name)
            ORDER BY total_count DESC {limit_clause_regulares}
        """
        ...
        if otu_seleccionada:
            opticos_where += """ AND Node IN (
                SELECT DISTINCT COALESCE(r.nombre, a.route_name)
                FROM mon_alarmas_veex a
                LEFT JOIN inv_rutas_fibra r ON r.id = a.ruta_obj_id
                LEFT JOIN inv_equipos_otu o ON o.id = r.otu_id
                WHERE COALESCE(o.nombre, a.device_serial) = %s
            )"""
        ...
        top_opticos_query = f"""
            SELECT
                Node AS Ruta,
                (SELECT COALESCE(o.nombre, a.device_serial)
                 FROM mon_alarmas_veex a
                 LEFT JOIN inv_rutas_fibra r ON r.id = a.ruta_obj_id
                 LEFT JOIN inv_equipos_otu o ON o.id = r.otu_id
                 WHERE COALESCE(r.nombre, a.route_name) = Node LIMIT 1) AS OTU,
...
def export_top_rutas_csv(request):
    ...
    where_clauses = " WHERE a.latitude BETWEEN -60.00 AND -17.00 AND a.longitude BETWEEN -76.00 AND -66.00"
    filter_params = []
    ...
    if anio_seleccionado:
        ...
        filter_params.append(anio_seleccionado)
    if otu_seleccionada:
        where_clauses += " AND COALESCE(o.nombre, a.device_serial) = %s"
        filter_params.append(otu_seleccionada)
    if ruta_seleccionada:
        where_clauses += " AND COALESCE(r.nombre, a.route_name) = %s"
        filter_params.append(ruta_seleccionada)
    ...
    top_rutas_query = f"""
        ...
        {where_clauses}
        GROUP BY COALESCE(r.nombre, a.route_name)
        ORDER BY total_count DESC {limit_clause}
    """
